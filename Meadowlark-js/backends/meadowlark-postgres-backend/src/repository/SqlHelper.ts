 ...
 */
export const createDatabaseSql = (sql: postgres.Sql<any>) => sql`CREATE DATABASE meadowlark`;

/**
 * SQL query string to create schema in the meadowlark database
 */

export const createSchemaSql = (sql: postgres.Sql<any>) => sql`CREATE SCHEMA IF NOT EXISTS meadowlark`;

/**
 * SQL query string to create document table
 */
export const createDocumentTableSql = (sql: postgres.Sql<any>) =>
  sql`CREATE TABLE IF NOT EXISTS meadowlark.documents(
...
export const createDocumentTableUniqueIndexSql = (sql: postgres.Sql<any>) =>
  sql`CREATE UNIQUE INDEX IF NOT EXISTS ux_meadowlark_documents ON meadowlark.documents(document_id)`;

/**
 * SQL query string to crate references table
 */
export function createReferencesTableSql(sql: postgres.Sql<any>) {
  return sql`
...
export function createReferencesTableCheckingIndexSql(sql: postgres.Sql<any>) {
  return sql`CREATE INDEX IF NOT EXISTS ix_meadowlark_references_checking ON meadowlark.references(referenced_document_id)`;
}
// For reference removal in transaction with parent update/delete
export function createReferencesTableDeletingIndexSql(sql: postgres.Sql<any>) {
  return sql`CREATE INDEX IF NOT EXISTS ix_meadowlark_references_deleting ON meadowlark.references(parent_document_id)`;
}
