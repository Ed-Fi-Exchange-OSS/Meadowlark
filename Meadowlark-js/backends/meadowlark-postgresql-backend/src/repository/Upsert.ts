...
export async function upsertDocument(upsertRequest: UpsertRequest, client: PoolClient): Promise<UpsertResult> {
  ...
  try {
    ...
    const upsertProcess = await retry(
      async (bail) => {
        ...
        try {
          ...
          Logger.debug(
            `${moduleName}.upsertDocument: Deleting references for document meadowlarkId ${meadowlarkId}`,
            traceId,
          );
          ...
          if (error.code === '40001') {
            ...
            Logger.error(`${moduleName}.upsertDocument`, traceId, error);
